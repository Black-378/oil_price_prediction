...
if button_clicked:
    ...
    forecast_dates = pd.date_range(start=pd.datetime.now().strftime('%Y-%m-%d'), end=end_date)
    ...
    # Specify the start date for forecasting as the last date in the historical data
    start_date_forecast = data.index[-1]
    
    # Forecast future oil prices
    forecast_dates = pd.date_range(start=start_date_forecast, periods=future_days + 1)
    forecast_scaled = []
    last_batch = X[len(X)-7]
    
    for _ in range(round(future_days + 1)):
        last_batch = last_batch.reshape(1, sequence_length, 1)
       # print("Input to model.predict:", last_batch)
        
        forecast = model.predict(last_batch)
        #print("Output from model.predict:", forecast)
    ...
    #print("Shape of forecast_scaled:", forecast_scaled.shape)
    #print("Values of forecast_scaled:", forecast_scaled)
    ...
    new_dates = []
    new_prices = []
    for dates, price in zip(forecast_dates, forecasted_prices):
        new_dates.append(dates.date())
        new_prices.append(price[0])
